...
inpatient_data_all_years= choose_columns(data_list = list(inpatient_data_2013,inpatient_data_2014,inpatient_data_2015,inpatient_data_2016,inpatient_data_2017,inpatient_data_2018,inpatient_data_2019,inpatient_data_2020),columns= c("DESY_SORT_KEY","CLM_THRU_DT","CLM_TOT_CHRG_AMT"))
...
yearly_calculator=function(data,time_frame=365,diagnosis){
    ...
    physician_integration_finder=function(data,integrated_place_of_service_codes=c("19","22"),all_place_of_service_codes=c("11","19","22"),integration_threshold=0.75,code_list=c("99201","99202","99203","99204","99205","99211","99212","99213","99214","99215")){
    ...
    physician_integration_stats=physician_integration_finder(data)
    #add to results
...
outpatient_data_all_years_stable_angina=inner_join(outpatient_data_all_years, yearly_calculations_stable_angina,by="DESY_SORT_KEY")
intpatient_data_all_years_stable_angina=inner_join(inpatient_data_all_years, yearly_calculations_stable_angina,by="DESY_SORT_KEY")

yearly_tot_outpatient_inpatient=function(data,time_frame=365){
    data%>%
      group_by(DESY_SORT_KEY)%>%
      filter(date>=first_diagnosis & first_diagnosis-date<time_frame)%>%
      summarise(tot_allowed=sum(CLM_TOT_CHRG_AMT))%>%
      as.data.table()
}
outpatient_tot_yearly=yearly_tot_outpatient_inpatient(outpatient_data_all_years_stable_angina)
inpatient_tot_yearly=yearly_tot_outpatient_inpatient(intpatient_data_all_years_stable_angina)
yearly_calculations_stable_angina=left_join(yearly_calculations_stable_angina, outpatient_tot_yearly, by="DESY_SORT_KEY")%>%as.data.table()
...
colnames(yearly_calculations_stable_angina)[c(3,24,25)]=c("tot_allowed_carrier","tot_allowed_outpatient","tot_allowed_inpatient")
...
#unstable angina
yearly_calculations_unstable_angina=yearly_calculator(carrier_data_all_years,diagnosis="is_unstable_angina")
outpatient_data_all_years_unstable_angina=inner_join(outpatient_data_all_years, yearly_calculations_unstable_angina,by="DESY_SORT_KEY")
intpatient_data_all_years_unstable_angina=inner_join(inpatient_data_all_years, yearly_calculations_unstable_angina,by="DESY_SORT_KEY")

outpatient_tot_yearly=yearly_tot_outpatient_inpatient(outpatient_data_all_years_unstable_angina)
inpatient_tot_yearly=yearly_tot_outpatient_inpatient(intpatient_data_all_years_unstable_angina)
yearly_calculations_unstable_angina=left_join(yearly_calculations_unstable_angina, outpatient_tot_yearly, by="DESY_SORT_KEY")%>%as.data.table()
...
colnames(yearly_calculations_unstable_angina)[c(3,24,25)]=c("tot_allowed_carrier","tot_allowed_outpatient","tot_allowed_inpatient")
