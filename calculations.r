...
yearly_calculator=function(data,time_frame=365,diagnosis){
    ...
    group_by(DESY_SORT_KEY)%>%
      filter(sum(eval(parse(text=diagnosis)),na.rm=T)==T)%>%
      mutate(first_diagnosis=min(date[eval(parse(text=diagnosis))==T]),na.rm = T)%>%
...
#stable angina
yearly_calculations_stable_angina=yearly_calculator(carrier_data_all_years,diagnosis="is_stable_angina")
